...
from name_format import NameFormat
init()
...
columns_visible = [0, 1, 2, 3, 6, 8, 9, 10, 11, 13, 15, 16, 17, 20]
...
# veerud, mille täitmisel programm proovib arvata sisu
# sõned on ainult koodi lugemise lihtsustamiseks
guessable_columns = {9:'Title', 10:'Artist', 11:'Album', 12:'Album Artist',
                     13:'Genre', 14:'Track Number', 15:'Year', 16:'Conductors',
                     17:'Composer', 18:'Writer'}
nameformat = NameFormat(columns)

# nimekiri zanritest, mille kasutaja on sisestanud
# sõnastikus zanr:Zanr, et ei korduks sama zanr väikse ja suure tähega
try:
    genres = {x.strip().lower():x.strip() for x in open('data/genres.txt')}
...
def enter():
    global current_dir, cursor_line, view_position, dir, message, genres
    ...
    if isCellEditable():
        ...
        # tag'i muutmine
            ...
            if audio != None:
                ...
                # andmete sisestamine
                # ette täidetakse see mis lahtris enne oli
                prefill = '; '.join(audio[mutagen_names[column]]) if mutagen_names[column] in audio else ''
                if columns[column] == 'Genre':
                    guess = list(genres.values())
                else:
                    guess = nameformat.guess(file[1][:file[1].rfind('.')], column)
                
                print('Name:', file[1][:file[1].rfind('.')])
                userin = special_input(columns[column] + ': ', prefill, guess)
                if columns[column] == 'Genre':
                    genres[userin.lower()] = userin
                
                try:
                    audio[mutagen_names[column]] = [x.strip() for x in userin.split(';')]
...
def main():
    ...
    global columns_visible, sorted_by, view_lines, view_position, cursor_column
    global home_fs, editing, cursor_line, dir
    ...
    while True:
        ...
        if ch:
            ...
            if ch == b'q':
                break
            elif ch in b_drives and getch() == b':': # Kõvaketta vahetus
